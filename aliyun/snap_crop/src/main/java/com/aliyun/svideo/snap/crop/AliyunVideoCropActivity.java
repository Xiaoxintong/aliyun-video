...
import com.aliyun.svideo.media.SizeChangedNotifier;
...
public class AliyunVideoCropActivity extends Activity implements TextureView.SurfaceTextureListener,
    ...
    private static int maxVideoDuration = Integer.MAX_VALUE;
    ...
    private void getData() {
        ...
        maxVideoDuration = getIntent().getIntExtra(AliyunSnapVideoParam.MAX_VIDEO_DURATION, 60000);
    ...
    private void startCrop() {
        if (mEndTime - mStartTime > AliyunVideoCropActivity.maxVideoDuration) {
            showLimitDialog();
            return;
        }
        ...
        int cropWidth;
        int cropHeight;
        outputPath = com.aliyun.svideo.common.utils.FileUtils.getHnxxtInnerDir(getApplicationContext(),"video") + DateTimeUtils.getDateTimeFromMillisecond(System.currentTimeMillis()) + Constants.SDCardConstants.CROP_SUFFIX;
...
//                }
    ...
    private void showLimitDialog() {
        int limitDuration = AliyunVideoCropActivity.maxVideoDuration / 1000;
        View contentView = LayoutInflater.from(AliyunVideoCropActivity.this).inflate(R.layout.crop_limit_dialog_layout, null);
        TextView tvDes = contentView.findViewById(R.id.tv_desc);
        //暂有问题，string放到string.xml中，这里运行时崩溃，找不到
        String tip = "仅支持上传"+limitDuration+"秒以内的视频，您可拖动下方进度条处的裁剪框调整视频时长";
        tvDes.setText(tip);
        TextView tvIKnow = contentView.findViewById(R.id.tv_i_know);
        final Dialog dialog = new Dialog(AliyunVideoCropActivity.this);
        dialog.setContentView(contentView);
        dialog.getWindow().setBackgroundDrawableResource(R.color.alivc_common_bg_transparent);
